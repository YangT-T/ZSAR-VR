...
class DInterface(pl.LightningDataModule):
    ...
    def __init__(self, **kwargs):
        ...
        self.vr_classes = kwargs.get("vr_classes", [])  # list of VR labels
        self.oversample_vr = kwargs.get("oversample_vr", False)
    ...
    def seen_train_dataloader(self):
        if self.oversample_vr:
            # 获取所有标签
            if hasattr(self.seen_train, 'labels'):
                labels = self.seen_train.labels
            else:
                labels = [self.seen_train[i][1] for i in range(len(self.seen_train))]

            # 为每个样本分配采样权重
            weights = []
            for label in labels:
                if label in self.vr_classes:
                    weights.append(5.0)  # VR类给予较高权重（可调）
                else:
                    weights.append(1.0)  # General类

            sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)

            return DataLoader(self.seen_train,
