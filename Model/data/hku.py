...
class Hku(data.Dataset):
    ...
    def load_video_feats(self, path):
        if self.sample_type == 'seen_train':
            feats = np.load(path + '/train_video_{}.npy'.format('feature'), allow_pickle=True)
        elif self.sample_type == 'seen_val':
            feats = np.load(path + '/val_video_{}.npy'.format('feature'), allow_pickle=True)
        elif self.sample_type == 'zsl_val':
            feats = np.load(path + '/test_video_{}.npy'.format('feature'), allow_pickle=True)
    ...
    def check_files(self):
        ...
        label_path = self.root2
        # seen_cls_num = self.cls_num - zsl_cls_num
        # get learning samples
        if self.sample_type == 'seen_train':
            true_class_ids = np.load('{}/label_splits/rs.npy'
                                     .format(self.root)).tolist()
            self.x, nonzero_idx = self.load_x(path)
            self.vfeats = self.load_video_feats(path)
            ...
            true_class_ids = np.load('{}/label_splits/rs.npy'.format(self.root)).tolist()
            self.x, nonzero_idx = self.load_x(path)
            self.vfeats = self.load_video_feats(path)
            self.y = self.load_y(np.load(label_path + '/val_labels.npy'))[nonzero_idx]
            # self.i = np.transpose(np.load(path + '/val_pimg.npy'), (0,3,1,2))
        elif self.sample_type == 'zsl_val':
            true_class_ids = np.load('{}/label_splits/ru.npy'.format(self.root)).tolist()
            self.x, nonzero_idx = self.load_x(path)
            self.vfeats = self.load_video_feats(path)
    ...
    def __getitem__(self, idx):
        if hasattr(self, "i"):
            raise NotImplementedError("未支持 pimg 数据")
        else:
            x_sample = self.x[idx]
            y_sample = self.y[idx]
            y2_sample = self.y2[idx]

            # 随机生成 video_token_feats: [1568, 768]
            # num_tokens = 1568
            # feat_dim = 768
            # video_token_feats = torch.randn(num_tokens, feat_dim)
            video_token_feats = self.vfeats[idx]


            if len(video_token_feats.shape) == 1:
                # 扩展成 T=16 个重复的 frame
                video_token_feats = np.tile(video_token_feats, (16, 1))


            return x_sample, y_sample, y2_sample, video_token_feats
