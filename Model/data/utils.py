...
def load_labels(root, split, dataloader, model_name):
    ...
    if dataloader=='hku':
        cls_num=7
        emb_dim = 512
        unseen_inds = np.load(os.path.join(root, 'label_splits/ru.npy'))
        seen_inds = np.load(os.path.join(root, 'label_splits/rs.npy'))

        cls_labels = np.load(os.path.join(root, 'hku_bpnames.npy'),allow_pickle=True)
    else:
