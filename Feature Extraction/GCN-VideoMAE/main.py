...
action_dict = {
    ...
    'shooting': '011',
    'bowling': '012',
    'move-using-controller': '013',
    'waive-sword': '014',
    'measure-length': '015',
    'picking-up-an-item-from-the-table-': '016',
    'throwing-a-net-to-catch-fish': '017',
    'grab-and-collect--box': '018'
}

# 构造 index -> 动作名 的映射（注意 -1）
id_to_action = {int(v) - 1: k for k, v in action_dict.items()}
...
class Processor():
    ...
    def load_data(self):
        ...
        print(len(self.data_loader['test']))
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            #     print(f"[Pre-clean] batch {batch_idx} has invalid values")
            ...
            # # 检查 AFTER 清理
            # if torch.isnan(data).any() or torch.isinf(data).any():
            #     print(f"[Post-clean] batch {batch_idx} still has invalid values")
            ...
            if torch.isnan(loss).any() or torch.isinf(loss).any():
                print("Loss is NaN or Inf. Check your data/model.")
                continue
    ...
    #             if torch.isnan(data).any() or torch.isinf(data).any():
    #                 print("Data is NaN or Inf. Check your data/model.")
    #                 data = torch.nan_to_num(data, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                for t, p in zip(true, pred):
                    ...
                    if result_file is not None:
                        f_r.write(f"{p},{t}\n")
                    if p != t and wrong_file is not None:
                        f_w.write(f"{index[t]},{p},{t}\n")
            ...
            if accuracy > self.best_acc:
                self.best_acc = accuracy
                score_dict = dict(zip(self.data_loader[ln].dataset.sample_name, score))
                with open(f'./work_dir/{arg.Experiment_name}/eval_results/best_acc.pkl', 'wb') as f:
                    pickle.dump(score_dict, f)

            print(f'Eval Accuracy: {accuracy:.4f}  model: {self.arg.model_saved_name}')
            self.print_log('\tMean {} loss of {} batches: {}.'.format(
                ln, len(self.data_loader[ln]), np.mean(loss_value)))
            for k in self.arg.show_topk:
                self.print_log('\tTop{}: {:.2f}%'.format(
            ...
            # with open(f'./work_dir/{arg.Experiment_name}/eval_results/epoch_{epoch}_{accuracy}.pkl', 'wb') as f:
            #     pickle.dump(score_dict, f)
